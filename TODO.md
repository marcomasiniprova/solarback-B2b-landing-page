...
- [x] **Setup profili social (docs/15)** + **Strategia contenuti/organic (docs/16)**: ricerca 4 agenti + piano completo (pilastri, hook, cadenza, per-piattaforma, KPI onesti, workflow avatar). **4 decisioni aperte per Valerio** (docs/16 §0): target realistico (800-1.800 a 6 mesi, non 2.5-10K), canale #1 da validare (LinkedIn vs gruppi FB), avatar-volto vs volto reale, autopilot human-in-the-loop (no full-auto engagement = ban).
- [ ] **PROMEMORIA accessi da Valerio (fase autopilot):** Unipile API + tool scheduling social (es. per pubblicazione multi-canale) + ElevenLabs (voce IT clonata) + HeyGen (avatar lip-sync) per i video coerenti.
