# reference.md · SOLAR - LINKEDIN DM OUTREACH
Contratto: `.claude/skills/solar-ruolo-template/reference.md`. MC = `https://mission-control-production-d22b.up.railway.app`.

## Q1 · Pool titolari con LinkedIn (Supabase `public`, project dziylyrneqeamqzatdzo)
```sql
select a.icp_tier, count(*) as n
from public.aziende a
where a.lista = 'Lista Target' and coalesce(a.titolare_linkedin, '') <> '' and a.stato = 'Contatto'
group by a.icp_tier order by a.icp_tier;
-- lista del giorno (N righe):
select a.id_sb, a.azienda, a.titolare_nome, a.titolare_cognome, a.titolare_linkedin, a.regione, a.icp_tier
from public.aziende a
where a.lista = 'Lista Target' and coalesce(a.titolare_linkedin, '') <> '' and a.stato = 'Contatto'
  and not exists (select 1 from mc.outreach_msgs o where o.channel = 'linkedin' and o.contact_ref = a.id_sb)
order by case a.icp_tier when 'A' then 1 when 'B' then 2 else 3 end, a.icp_score desc nulls last
limit :n;
```
## Stato (kv `linkedin:stato`)
...
1. **Richiesta di collegamento** (≤ 300 caratteri): chi siamo in una riga + perché lui (installatore FV nella sua zona), nessuna vendita.
...
- Base URL raggiungibile dal proxy: `https://api65.unipile.com/api/v1/...?port=19505` (auth iniettata dal connettore). Endpoint da verificare al primo uso: `GET /accounts`, `GET /chats`, `GET /chats/{id}/messages`, `POST /chats` (nuovo messaggio), `POST /users/invite` (collegamento).
- Ogni invio → `outreach_add direction:"out"`; ogni risposta → `outreach_add direction:"in"` + risposta entro il giro; meeting → `meeting_add`.
- Volumi: quelli in `linkedin:stato.volumi`, distribuiti sulle 12 ore (mai tutti in un giro). Stop immediato se LinkedIn mostra avvisi/restrizioni: `feed` kind `error`.
